...
object Netlify:
  ...
  def listRecursively[F[_]: {Async, Files}](root: Path): Stream[F, Path] =
    ...
    F.list(root)
      .flatMap: p =>
        Stream
          .eval(F.isRegularFile(p))
          .flatMap: isRegular =>
            if isRegular then Stream.emit[F, Path](p)
            else listRecursively(p)
...
class Netlify[F[_]: {Async, Files, Concurrent, Hashing}](token: AccessToken, http: HttpClientF2[F]):
  ...
  private def uploadables(in: FileDigests, site: SiteId): F[Uploadables] =
    http
  ...
  private def uploadFile(path: String, deployId: DeployId, file: Path) =
